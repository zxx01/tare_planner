...
namespace misc_utils_ns
{
  ...
  /**
   * @brief 检查一个点是否在给定视点的视野内。函数根据点的位置、视点的位置以及一些阈值参数来判断点是否在视野内。
   *
   * @param point_position      给定的待判断的点的位置
   * @param viewpoint_position  视点的位置
   * @param vertical_fov_ratio  垂直视野角的比率，用于限制点在垂直方向上的可见范围
   * @param range               传感器的可视范围，用于限制点与视点之间的水平距离
   * @param xy_dist_threshold   水平方向上的距离阈值，用于近似判断点是否在视野内。
   * @param z_diff_threshold    垂直方向上的高度差阈值，用于近似判断点是否在视野内。
   * @param print               布尔值，用于控制是否打印调试信息
   * @return true
   * @return false
   */
  ...
  {
    // 计算给定点与视点的水平距离：计算点与视点在水平方向上的距离，即点在 xy 平面上的投影距离。
    Eigen::Vector3d diff = point_position - viewpoint_position;
    double xy_dist = sqrt(diff.x() * diff.x() + diff.y() * diff.y());

    // 如果点在 xy 方向上的距离小于设定的阈值，并且点在 z 方向上的高度差小于设定的阈值，说明点在视点附近，可能在视野内。
    ...
    // 如果点在 xy 方向上的距离大于设定的传感器范围 range，则说明点在视点的水平范围之外，不在视野内。
    if (xy_dist > range)
    {
      if (print)
      {
    ...
    // 如果点在 z 方向上的高度差超过垂直视野角的比率乘以点在 xy 方向上的距离，则说明点在视点的垂直范围之外，不在视野内。
  ...
  /**
   * @brief 简化给定路径中的连续点。该函数的目的是减少路径中冗余的点，只保留不在同一直线上的点。
   *
   * @param path
  ...
  void UniquifyIntVector(std::vector<int> &list)
  {
    ...
    it = std::unique(list.begin(), list.end()); // 对排序后的向量进行去重操作。这会移除连续重复的元素，使得每个元素只保留一个。
