...
namespace lidar_model_ns
{
  ...
  public:
    ...
    /**
     * @brief 根据给定的点更新传感器模型中的点云信息。它计算给定点到传感器模型的距离，根据水平和垂直角度以及邻居数量更新传感器模型中的覆盖信息。
     * TODO
     * @tparam PointType
     * @param point
     */
    ...
    void UpdateCoverage(const PointType &point)
    {
      ...
      // 遍历水平和垂直方向上的邻居格子，用于更新传感器模型中的点云覆盖信息。
      for (int n = -horizontal_neighbor_num; n <= horizontal_neighbor_num; n++)
      {
        ...
        for (int m = -vertical_neighbor_num; m <= vertical_neighbor_num; m++)
        {
          ...
          // 更新覆盖信息：检查当前格子中存储的先前距离信息，
          // 如果先前距离为零，或者新的距离小于先前的距离，或者格子被标记为需要重置，则更新格子中的距离信息并取消格子的重置标记。
    ...
    /**
     * @brief 检查给定的点是否在传感器模型的视野内。根据点的距离和角度信息，判断点是否在传感器的视野范围内。
     * TODO
     * @tparam PointType
     * @param point
  ...
  private:
     ...
     *        (根据给定的点到传感器模型的距离，计算在水平方向上的'单侧'邻居数，以适应点云分辨率和水平分辨率。)
     * @param distance_to_point
     * @return int
     */
     ...
     *        (根据给定的点到传感器模型的距离，计算在竖直方向上的'单侧'邻居数，以适应点云分辨率和竖直分辨率。)
     * @param distance_to_point
     * @return int
     */
